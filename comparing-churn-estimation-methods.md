...
Customer relationship management is a strategy based on of building, managing and strengthen the relationship with customers (Vafeiadis, Diamantaras, Sarigiannidis, & Chatzisavvas, 2015). Predicting churn and preventing customers from churning can save companies hundreds of thousands dollars (Neslin, Gupta, Kamakura, Lu & Mason, 2006). The cost of attracting new customers is twelve times the cost of retaining your existing customers. Moreover, service providers can no longer rely on a steady customer base. In telecom service industries churn rates range from 23.4% to 46% (Neslin, et al., 2006). Where churn is defined as a customer’s telecommunications service is cancelled by a customer-initiated action (Nie, Rowe, Zhang, Tian & Shi, 2011). This makes customer churn a significant problem for companies. Hence, getting a clear view of which customers are likely to churn enables companies to focus on customers that are likely to churn and reactivate them (Neslin, et al, 2006). This study uses a dataset from an anonymous telecom provider.

# Problem statement
Numerous studies have described methods for predicting customer churn. Predicting customer churn contains a decision in terms of a nominal variable, where zero describes no cancellation of the contract and one means the customer cancelled the contract. Methods that become relevant when the dependent variable, churn or no churn, is a binary decision are decision trees, logistic regression, neural networks, random forest and support vector machines, with or without bagging and boosting (Neslin, et al., 2006: Günther, Tvete, Aas, Sandnes & Borgan, 2011: Vafeiadis, et al., 2015). Respectively, all of these techniques have predictive power for predicting customer churn. However, there is little knowledge of modern techniques of predictive customer churn. Modern techniques need to be explored to find methodologies that might have more predictive power (Neslin, et al., 2006: Risselada, Verhoef & Bijmolt, 2010). 
...
Another definition by Risselada, Verhoef & Bijmolt (2010) is the termination of a contract. More specific, in the telecommunications industry, the definition of churn is defined as; the action that a customer’s telecommunications service is cancelled by a customer-initiated action (Nie, Rowe, Zhang, Tian & Shi, 2011).

## Heavy users
Heavy users are described by Kim & Yoon (2004) as the total calls a user makes in the subscription service. The calls a user makes has a significant effect on the probability to churn of customers (Kim & Yoon, 2004: Ahn, Han, & Lee, 2006). In the dataset, these variables are expressed as the total minutes and total calls on an interval scale.

## Account length
The account length is defined as the time a customer has a subscription to the service providers, measured on an interval scale. Customers with a long account length are more likely to stay at the company due to switching costs (Kim & Yoon, 2004). Kim & Yoon (2004) suggest that the duration of a subscription has an effect on the user behaviour and therefore also on the probability to churn.

## Costs
According to Nie et al. (2011), revenue from customers influences the likelihood to churn. High costs for customers might be a cause for consideration to switch or cancel the contract with the current provider (Kim & Yoon, 2004). The revenue is expressed in a ratio variable in the dataset.
...
## Logistic regression
A logistic regression is one of the most used approaches to estimate the probability to churn (Neslin, et al., 2006: Risselada, Verhoef & Bijmolt, 2010). A logistic regression, also referred to as a logit, is similar to a linear regression. Similar in a way that, a logistic regression is also able to test for hypothesis. This implies that a logistic regression is able to test for significant variables, whereas a decision tree, bagging and boosting, neural networks are only able to present variable importance (Bischop, 2006). Moreover, a regression is able to output continuous variables, whereas a logistic regression only estimates the probability belonging to a group between 0 and 1 due to its sigmoid nature (Malhotra, 2010: Vafeiadis, et al., 2015). 
Consider that churn has two outcomes: the customer churned and the customer did not churn. When the predicted value is greater than 0.5, then the predicted value of Y is set to 1, when the value is below 0.5 the predicted value is set to 0. The probability of a customer churning is modelled using the logit model (Bishop, 2006: Malhotra, 2010). 
...
With the definition of entropy, the next step is to define information gain. Mitchell (1997) defines information gain as: “the measure of the effectiveness of an attribute in classifying the training data”. Information gain is based on the decrease in entropy, to find the attribute that returns the highest information gain. The information gain for all attributes are calculated, which forms the hierarchy of the attributes in the decision tree (Mitchell, 1997: Bishop, 2006).
...
## Logistic regression 
In table 1, two logistic models are presented. The first logistic estimation includes all the available variables in the telecom dataset. The Akaike information criterion or AIC, which indicates a goodness of fit of the logistic regression, has a value of 1633 and was able to predict 84.65% of all the customers correctly (Akaike, 1974). Removing all the insignificant variables resulted in the estimation presented in model two. This estimation has an AIC value of 1627.2 and is able to predict 84,89% of all customers correctly. Therefore, model two is used to compare the logistic regression with the decision tree, bagging, boosting and a neural network. 

Table 1, estimation of the logistic regression
	Model 1		Model 2
	B	Std. Error			B	Std. Error	
(Intercept)	-8.68	.83	***	 	-8.48	.82	***
Account Length	.0014	.00			-		
Total Charge	25.24	22.85			.33	.10	**
Total Calls	-.08	.03	**		-.08	.03	**
Total Minutes	-6.74	6.17			-.01	.00	*
International Plan	2.05	.17	***		2.05	.17	***
Voicemail Plan	-1.81	.66	**		-.92	.17	***
Voicemail Message	.03	.02			-		
Daytime Minutes	2.63	7.30			-.03	.01	*
Daytime Calls 	.08	.03	**		.08	.03	**
Daytime Charge	-1.00	32.33			-		
Evening Minutes	6.56	6.50			-.01	.00	*
Evening Calls	.09	.03	**		.08	.03	**
Evening Charge	-23.04	32.31			-		
Night Minutes	5.69	6.25			-		
Night Calls	.08	.03	**		.08	.03	**
Night Charge	-1.78	32.35			-		
Customer Service Calls	.51	.05	***	 	.51	.04	***
Gini Coefficient	.66		.67
TDL	3.27		3.35
AIC	1633		1627.2
Hit score	0.85	 	0.85
...
## Decision tree
By training all the variables available in the dataset, the decision tree presented in figure 1 is estimated.




Figure 1, variable importance of the decision tree
Based on information gain, splitting the dataset based on the total a customer was charged lead to the highest information gain, as presented in figure 1. For example: when a customer is charged more than 74 dollars, the customer has a .91 likelihood to churn. If a customer is charged less or equal than 74 dollars the likelihood to churn is .74, as displayed in figure 2. Followed by the amount of customer service calls a customer performed and the voice mail plan a customer possesses.
